    ...
    stages {
        stage('Setup env') {
            ...
            steps {
                ...
                //     // env.IMAGE_EXISTS = sh(label: 'Check if image is already pushed', returnStatus: true, script: "docker --config ./ manifest inspect us.icr.io/apihub-cr/didact-builder-service:${env.IMAGE_TAG}")
                // }
            }
        }
        ...
        stage('Build') {
            ...
            steps {
                sh label: 'Build', script: "docker --config ./ build -t  us.icr.io/apihub-cr/dev-pg/playground-theia-utilities:latest ."
                sh label: 'Push Image', script: "docker --config ./ push  us.icr.io/apihub-cr/dev-pg/playground-theia-utilities:latest"
            }
        }
        ...
        //             yq e -i '(.images.[] | select(.name == "us.icr.io/apihub-cr/didact-builder-service").newTag) |= "${env.IMAGE_TAG}"' ${params.ENV}/resources/kustomization.yaml
